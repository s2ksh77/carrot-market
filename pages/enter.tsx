...
const Enter: NextPage = () => {
  const [navTab, setNavTab] = useState<'email' | 'phone'>('email');
  const handleClickEmail = () => setNavTab('email');
  const handleClickPhone = () => setNavTab('phone');
  return (
      ...
      <div className="mt-8">
          ...
          <div className="grid border-b w-full grid-cols-2 gap-16 mt-8">
            <button
              className={cls(
                ...
                navTab === 'email' ? ' border-orange-500 text-orange-400' : 'border-transparent text-gray-500',
              )}
            ...
            <button
              className={cls(
                'pb-4 font-medium border-b-2 ',
                navTab === 'phone' ? 'border-orange-500 text-orange-400' : 'border-transparent text-gray-500',
              )}
        ...
        <form className="flex flex-col mt-8">
          ...
          <div className="mt-2">
            ...
            {navTab === 'phone' ? (
              <div className="flex" rounded-md shadow-sm>
                <span className="flex items-center justify-center textm-sm px-3 rounded-l-md border border-r-0 border-gray-300 gb-gray-50 text-gray-500 select-none">
                  +82
                </span>
                <input
                  id="input"
          ...
          <button className="mt-5 bg-orange-500 hover:bg-orange-600 text-white py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium focus:ring-2 focus:ring-offset-2 focus:ring-orange-500 focus:outline-none ">
            {navTab === 'email' ? '인증메일 받기' : null}
            {navTab === 'phone' ? '인증문자 받기' : null}
          </button>
        </form>
        <div className="mt-8">
          <div className="relative">
            ...
            <div className="relative -top-3 text-center ">
              <span className="bg-white px-2 text-sm text-gray-500">또는 다른 로그인</span>
            </div>
